...
const LocationFeed = ({ userLocation, locationError, onSelectIssue, isDarkMode, session }) => {
  ...
  }, [userLocation, locationError, feedType, session]);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto flex flex-col gap-6">
      <div className={`flex flex-col gap-4 border-b-4 pb-4 ${isDarkMode ? 'border-white' : 'border-black'}`}>
        {locationError && (
          <div className="bg-[#FFCC00] text-black border-4 border-black p-3 font-mono font-black text-xs sm:text-sm uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] text-center">
            ⚠️ GPS ACCESS DISABLED // GLOBAL FEED SHOWN
          </div>
        )}
        <div className="flex justify-between items-end">
          <div>
            <h1 className="text-4xl font-black uppercase tracking-tight leading-none whitespace-normal break-words">
              {feedType === 'local' ? 'Local Feed' : feedType === 'global' ? 'Global Feed' : 'Verification'}
            </h1>
            <p className="font-mono font-bold uppercase mt-2 whitespace-normal break-words">
      ...
      {loading ? (
        ...
        <div className={`relative border-4 rounded-3xl text-center p-8 mt-4 overflow-hidden ${isDarkMode ? 'border-white bg-zinc-900 shadow-[6px_6px_0px_0px_rgba(255,255,255,1)]' : 'border-black bg-white shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]'}`}>
          ...
          <p className="font-black uppercase text-xl relative z-10 whitespace-normal break-words">No issues found {feedType === 'local' ? 'within 1km' : 'globally'}.</p>
        </div>
      ) : (
        <div className="w-full max-w-2xl mx-auto flex flex-col gap-6 items-center mt-2">
          {issues.map(issue => {
            ...
            if (!hasMedia) {
              return (
                ...
                >
                  ...
                  <p className={`font-mono text-xs font-bold uppercase mb-2 whitespace-normal break-words ${isDarkMode ? 'text-gray-300' : 'text-gray-800'}`}> {issue.location_name}</p>
                  <h3 className={`text-lg sm:text-xl font-black uppercase tracking-tight whitespace-normal break-words ${issue.status === 'SOLVED' ? 'line-through' : ''} ${isDarkMode ? 'text-white' : 'text-black'}`}>{issue.category} Report</h3>
                  <p className={`font-mono text-sm mt-2 whitespace-normal break-words ${issue.status === 'SOLVED' ? 'line-through' : ''} ${isDarkMode ? 'text-gray-200' : 'text-black'}`}>{issue.description || issue.ai_description}</p>
            ...
            return (
              <div 
                key={issue.id} 
                className={`relative flex flex-col w-full box-border mb-6 border-4 hover:-translate-y-1 hover:-translate-x-1 active:translate-y-0.5 active:translate-x-0.5 transition-all cursor-pointer rounded-3xl mt-4 ${issue.status === 'SOLVED' ? 'opacity-75 grayscale-[0.2]' : ''} ${isDarkMode ? 'bg-black border-white shadow-[6px_6px_0px_0px_rgba(255,255,255,1)]' : 'bg-white border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]'}`}
                onClick={() => onSelectIssue(issue.id)}
              >
                ...
                <div className="p-4 flex flex-col gap-2">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col">
                        <span className="font-bold text-sm uppercase leading-none whitespace-normal break-words">{issue.reporter_name || 'Anonymous'}</span>
                  ...
                  <div className="mt-2 flex flex-col items-start gap-2">
                    <p className={`font-mono text-xs font-bold uppercase mt-1 whitespace-normal break-words ${isDarkMode ? 'text-gray-300' : 'text-gray-800'}`}> {issue.location_name}</p>
                  </div>
                </div>
                ...
                <div className="p-4 flex flex-col space-y-2">
                  <p className={`font-mono text-sm whitespace-normal break-words ${issue.status === 'SOLVED' ? 'line-through' : ''} ${isDarkMode ? 'text-gray-200' : 'text-black'}`}>{issue.description || issue.ai_description}</p>
                  <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mt-3 sm:mt-4">
                    <span className={`font-mono text-[10px] font-bold uppercase border-2 px-2 py-0.5 ${isDarkMode ? 'border-white bg-zinc-900 text-white' : 'border-black bg-white text-black'}`}>
                      UPVOTES: {issue.upvote_count}
                    </span>
