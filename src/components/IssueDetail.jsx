...
const IssueDetail = ({ issueId, userLocation, onBack, isDarkMode, session }) => {
  ...
  const userLocality = userProfile?.localArea?.toLowerCase().trim();
  const userPin = userProfile?.pinCode?.trim();
  const issueLocality = issue.reportedLocality?.toLowerCase().trim();
  const issuePin = issue.reportedPIN?.trim();

  const isLocalResident = (userLocality && issueLocality && userLocality === issueLocality) || (userPin && issuePin && userPin === issuePin);
  const notLocal = !isLocalResident;
          ...
          {!isEscalated && !isSolved && !isUnderProcess && (
            <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto relative group">
              <button
                ...
                disabled={notLocal || upvoting || isPoster || downvoting}
                ...
                title={isPoster ? "You cannot co-sign your own issue." : notLocal ? "You must be a local resident to co-sign it." : ""}
              >
              ...
              <button
                ...
                disabled={notLocal || downvoting || isPoster || upvoting}
                ...
                title={isPoster ? "You cannot flag your own issue." : notLocal ? "You must be a local resident to flag it." : ""}
              >
